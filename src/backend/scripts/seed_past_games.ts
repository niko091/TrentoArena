...
const getElo = (user: IUser, sportId: string): number => {
    const entry = user.sportsElo?.find((e: any) => e.sport.toString() === sportId);
    return entry ? entry.elo : INITIAL_ELO;
};

const updateElo = (user: IUser, sportId: string, newElo: number, date: Date, change: number) => {
...
const seedGames = async () => {
    try {
        ...
        for (const gameData of gamesToProcess) {
            ...
            // Update Users
            updateElo(winnerUser, sportId, newWinnerElo, gameData.date, newWinnerElo - winnerElo);
            updateElo(loserUser, sportId, newLoserElo, gameData.date, newLoserElo - loserElo);
