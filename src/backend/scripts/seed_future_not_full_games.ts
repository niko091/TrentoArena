...
const seedGames = async () => {
    try {
        ...
        if (users.length < 10) throw new Error('Not enough users to simulate games with up to 10 participants.');
        ...
        for (let i = 0; i < GAMES_TO_SIMULATE; i++) {
            ...
            const numParticipants = faker.number.int({ min: 1, max: maxParticipants - 1 });
        ...
        for (const gameData of gamesToProcess) {
            ...
            await Game.create({
                ...
                participants: gameData.participants.map((u: any) => ({ user: u._id, winner: false }))
            });

            processedCount++;
            if (processedCount % 10 === 0) console.log(`Processed ${processedCount} games...`);
        }
