...
async function fetchGames() {
    try {
        ...
        games.forEach(game => {
            ...
            const sportName = game.sport ? game.sport.name : window.i18n.t('admin.js.unknown');
            const placeName = game.place ? game.place.name : window.i18n.t('admin.js.unknown');
            const creatorName = game.creator ? game.creator.username : window.i18n.t('admin.js.unknown');
            ...
            row.innerHTML = `
                        ...
                        <td>${formattedTime}</td>
                        <td>${creatorName}</td>
                        <td>${game.isFinished ? window.i18n.t('admin.js.yes') : window.i18n.t('admin.js.no')}</td>
...
async function fetchPlaces() {
    try {
        ...
        places.forEach(place => {
            const row = document.createElement('tr');
            const sportName = place.sport && place.sport.name ? place.sport.name : window.i18n.t('admin.js.unknown');
...
async function deletePlace(id) {
    if (!confirm(window.i18n.t('admin.js.confirm_delete_place'))) {
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            alert(window.i18n.t('admin.js.place_deleted'));
            fetchPlaces();
        } else {
...
async function fetchSports() {
    try {
        ...
        select.innerHTML = `<option value="" disabled selected>${window.i18n.t('profile.select_sport')}</option>`;
...
async function fetchReports() {
    try {
        ...
        reports.forEach(report => {
            ...
            const reporterName = report.reporter ? report.reporter.username : window.i18n.t('admin.js.unknown');
            const reportedName = report.reported ? report.reported.username : window.i18n.t('admin.js.unknown');
...
async function fetchBannedUsers() {
    try {
        ...
        users.forEach(user => {
            ...
            let expiryDisplay = window.i18n.t('admin.js.forever');
            let remainingDisplay = window.i18n.t('admin.js.forever');
            ...
            if (user.banExpiresAt) {
                ...
                if (diffMs > 0) {
                    ...
                    remainingDisplay = window.i18n.t('admin.js.expired');
                }
            }
...
async function unbanUser(userId, username) {
    if (!confirm(window.i18n.t('admin.js.confirm_unban', { username }))) return;
    ...
    try {
        ...
        if (response.ok) {
            alert(window.i18n.t('admin.js.user_unbanned', { username }));
            fetchBannedUsers();
        } else {
            const data = await response.json();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                alert(window.i18n.t('admin.js.place_added'));
                form.reset();
    ...
    sportForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                alert(window.i18n.t('admin.js.sport_added'));
                sportForm.reset();
    ...
    editForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                alert(window.i18n.t('admin.js.place_updated'));
...
window.openEditModal = async function (id) {
    try {
        ...
        sportSelect.innerHTML = `<option value="" disabled>${window.i18n.t('profile.select_sport')}</option>`;
        sports.forEach(s => {
            const option = document.createElement('option');
            option.value = s._id;
        ...
        if (!place) {
            alert(window.i18n.t('admin.js.place_not_found'));
            return;
        }
