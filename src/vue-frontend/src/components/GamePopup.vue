...
const isCreator = computed(() => {
    return props.currentUser && (props.game.creator._id || props.game.creator) === props.currentUser._id;
});

const isParticipant = computed(() => {
    return props.currentUser && props.game.participants && props.game.participants.some((p: any) => {
...
const isFull = computed(() => props.game.participants && props.game.maxParticipants && props.game.participants.length >= props.game.maxParticipants);
const isGameTimePassed = computed(() => new Date() >= new Date(props.game.date));

const formattedDate = computed(() => new Date(props.game.date).toLocaleDateString(undefined, { year: 'numeric', month: 'numeric', day: 'numeric' }));
const formattedTime = computed(() => new Date(props.game.date).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }));
...
async function joinGame() {
    ...
    } catch (e) { console.error(e); alert(t('game_popup.connection_error')); }
...
async function deleteGame() {
    ...
    loadingAction.value = true;
    try {
        const res = await fetch(`/api/games/${props.game._id}`, { method: 'DELETE' });
        if (res.ok) {
        ...
        <div class="game-popup-content">
            ...
            <div class="game-popup-body">
                ...
                <div class="game-popup-row">
                    ...
                    <span class="game-popup-value">{{ game.participants ? game.participants.length : 0 }} / {{ game.maxParticipants || '?' }}</span>
                </div>

                <!-- Participants List -->
                <div v-if="game.participants && game.participants.length > 0" class="game-popup-section">
                    <h4 class="game-popup-subtitle">
                        {{ t('game_popup.players') }}
                        <small v-if="isCreator && !isFinished" class="fw-normal text-muted ms-2" style="font-size: 0.7em;">({{ t('game_popup.check_winners') }})</small>
                    </h4>
                    <div class="game-popup-participants-list">
                        <div v-for="p in game.participants" :key="p.user._id || p.user" class="participant-item" :title="p.user.username">
                            
                            <!-- Winner Checkbox (Creator & Active) -->
                            <input v-if="isCreator && !isFinished" 
                                   type="checkbox" 
                            ...
                            <img :src="p.user.profilePicture || '/images/utenteDefault.png'" 
                                 class="participant-avatar me-2">
                            
                            <a :href="`/user/${p.user.username}`" class="participant-name text-inherit">{{ p.user.username }}</a>
                        </div>
                    </div>
                </div>
                ...
                <!-- Actions -->
                <div class="popup-actions mt-4 d-flex justify-content-center flex-column gap-2">
                    <!-- Join Button -->
                    <button v-if="currentUser && !isFinished && !isFull && !isParticipant" 
                    ...
                    <div v-if="isParticipant" class="text-center">
                        <span class="badge bg-success py-2 fs-6">{{ t('game_popup.is_participant') }}</span>
                    </div>
                    <div v-else-if="isFinished" class="text-center">
                        <span class="badge bg-secondary py-2 fs-6">{{ t('game_popup.finished') }}</span>
                    </div>
                    <div v-else-if="isFull" class="text-center">
                        <span class="badge bg-danger py-2 fs-6">{{ t('game_popup.full') }}</span>
                    </div>
...
[data-bs-theme="dark"] .game-popup-close { color: #adb5bd; }
[data-bs-theme="dark"] .game-popup-close:hover { color: #fff; }
[data-bs-theme="dark"] .game-popup-label { color: #adb5bd; }
[data-bs-theme="dark"] .game-popup-value { color: #fff; }
[data-bs-theme="dark"] .game-popup-title { color: #fff; }
