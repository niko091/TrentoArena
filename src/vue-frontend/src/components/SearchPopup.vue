...
<template>
    <div class="search-popup-overlay" :class="{ 'active': isActive }" @click.self="closePopup">
        <div class="search-popup-content">
            ...
            <div class="search-results-section">
                <!-- Initial State -->
                <div v-if="!hasSearched && searchQuery.trim().length === 0" class="no-results">
                    {{ t('search_popup.start_typing') }}
                </div>

                <!-- Type More State -->
                <div v-if="!hasSearched && searchQuery.trim().length > 0 && searchQuery.trim().length < 2" class="no-results">
                ...
                <div v-else-if="hasSearched">
                    <div v-if="resultsUsers.length === 0 && resultsPlaces.length === 0" class="no-results">
                        {{ t('search_popup.no_results') }}
                    </div>

                    <div v-if="resultsUsers.length > 0">
                        <div class="section-title">{{ t('search_popup.users') }}</div>
                        <a v-for="user in resultsUsers" :key="user.username" :href="`/user/${user.username}`" class="result-item">
                            <img :src="user.profilePicture || '/images/utenteDefault.png'" class="result-img" :alt="user.username">
                    ...
                    <div v-if="resultsPlaces.length > 0">
                         ...
                         <a v-for="place in resultsPlaces" :key="place._id" :href="`/map?placeId=${place._id}`" class="result-item">
