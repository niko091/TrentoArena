...
const getBorderClass = (game: any) => {
  ...
  const userParticipation = game.participants.find((p: any) => 
    (p.user._id || p.user) === props.profileUserId
  );
  
  return userParticipation?.winner ? 'border-success border-2' : 'border-danger border-2';
};
  ...
  <div class="mt-4 mt-lg-5">
    ...
    <div class="row g-3">
      <div v-if="games.length === 0" class="col-12">
        <span class="text-muted">{{ isPast ? t("profile.no_past_games") : t("profile.no_upcoming_games") }}</span>
      </div>
          ...
          <div class="card-body">
            <div class="d-flex justify-content-between mb-2">
              ...
              <template v-if="isPast">
                 <span v-if="getBorderClass(game).includes('success')" class="badge bg-success">
                   {{ t("profile.won") }}
                 </span>
