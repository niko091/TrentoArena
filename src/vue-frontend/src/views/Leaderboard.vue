...
const displayedUsers = computed(() => {
    if (filterType.value === 'friends' && currentUser.value) {
        ...
        return leaderboardData.value.filter(u => 
            u._id === currentUser.value?._id || friendIds.includes(u._id)
        );
    }
            ...
            <div class="col-md-8">
                <div class="card shadow-sm border-0">
                    <div class="card-body p-4">
                        <h2 class="text-center fw-bold mb-4" style="color: rgb(223, 103, 5);">
                            {{ t('leaderboard.title') }}
                        </h2>

                        <div class="d-flex justify-content-center mb-4">
                            <div class="btn-group" role="group" aria-label="Leaderboard Type">
                                <input type="radio" class="btn-check" name="lbType" id="lbGlobal" autocomplete="off"
                                    value="global" v-model="filterType">
                                <label class="btn btn-outline-primary" for="lbGlobal">
                                    {{ t('leaderboard.global') }}
                                </label>

                                <input type="radio" class="btn-check" name="lbType" id="lbFriends" autocomplete="off"
                                    value="friends" v-model="filterType" :disabled="!currentUser">
                                <label class="btn btn-outline-primary" for="lbFriends"
                        ...
                        <div class="mb-4">
                            ...
                            <select id="sportFilter" class="form-select" v-model="selectedSportId">
                            ...
                            <table class="table table-hover align-middle">
                                <thead class="">
                                    <tr>
                                        ...
                                        <th scope="col" style="width: 70%;">{{ t('leaderboard.col_player') }}</th>
                                        <th scope="col" style="width: 20%;" class="text-end">{{ t('leaderboard.col_elo') }}</th>
                                    </tr>
                                </thead>
                                <tbody id="leaderboardTableBody">
                                    ...
                                    <!-- Error State -->
                                    <tr v-else-if="error">
                                        <td colspan="3" class="text-center py-4 text-danger">{{ error }}</td>
                                    </tr>

                                    <!-- Empty State (No sport selected or no results) -->
                                    <tr v-else-if="!selectedSportId">
                                        <td colspan="3" class="text-center py-4 text-muted">{{ t('leaderboard.select_sport_prompt') }}</td>
                                    </tr>
                                    <tr v-else-if="displayedUsers.length === 0">
                                        <td colspan="3" class="text-center py-4 text-muted">{{ t('leaderboard.none_found') }}</td>
                                    </tr>

                                    <!-- Results -->
                                    <tr v-else v-for="(user, index) in displayedUsers" :key="user._id">
                                        <!-- Rank -->
                                        <td class="align-middle" v-if="index === 0"><span class="badge bg-warning text-dark fs-6">🥇 1</span></td>
                                        <td class="align-middle" v-else-if="index === 1"><span class="badge bg-secondary fs-6">🥈 2</span></td>
                                        <td class="align-middle" v-else-if="index === 2"><span class="badge text-bg-custom-bronze fs-6" style="background-color: #cd7f32; color: white;">🥉 3</span></td>
                                        <td class="align-middle" v-else><strong>#{{ index + 1 }}</strong></td>

                                        <!-- Player -->
                                        <td class="align-middle">
                                            <div class="d-flex align-items-center">
                                                <img :src="user.profilePicture || '/images/utenteDefault.png'" 
                                                     :alt="user.username" 
                                                     class="leaderboard-img shadow-sm">
                                                <router-link :to="`/user/${user.username}`" class="text-decoration-none fw-semibold" style="color: inherit;">
                                                    {{ user.username }}
                                                </router-link>
                                            </div>
                                        </td>
                                        ...
                                        <!-- ELO -->
                                        <td class="align-middle text-end fw-bold font-monospace fs-5" style="color: var(--accent-primary, #fd7e14);">
                                            {{ user.elo }}
                                        </td>
                                    </tr>
                                </tbody>
