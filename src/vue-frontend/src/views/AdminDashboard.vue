        ...
        <div class="container py-4">
            ...
            <div class="row g-4">
                    ...
                    <div class="card h-100 border-0 shadow-sm">
                        ...
                        <div class="card-body">
                            <form @submit.prevent="addPlace">
                                ...
                                <div class="mb-3">
                                    <label for="sport" class="form-label">{{ t('admin.sport') }}</label>
                                    <select id="sport" v-model="addPlaceForm.sport" class="form-select" required>
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="lat" class="form-label">{{ t('admin.lat') }}</label>
                                        <input type="number" id="lat" v-model="addPlaceForm.lat" step="any" class="form-control" placeholder="46.0..." required>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="lng" class="form-label">{{ t('admin.lng') }}</label>
                                        <input type="number" id="lng" v-model="addPlaceForm.lng" step="any" class="form-control" placeholder="11.1..." required>
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-success w-100 text-white fw-bold">{{ t('admin.add_place_btn') }}</button>
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-6">
                    <div class="card h-100 border-0 shadow-sm">
                        <div class="card-header bg-white py-3">
                            <h2 class="h5 mb-0 text-primary">{{ t('admin.existing_sports') }}</h2>
                        </div>
                        <div class="card-body">
                            <div class="mb-4">
                                <label class="form-label">{{ t('admin.add_new_sport') }}</label>
                                <form @submit.prevent="addSport" class="d-flex gap-2">
                                    <input type="text" v-model="addSportName" class="form-control" :placeholder="t('admin.sport_name_placeholder')" required>
                                    <button type="submit" class="btn btn-primary text-white">{{ t('admin.add_btn') }}</button>
                                </form>
                            </div>

                            <div>
                                <h3 class="h6 text-secondary mb-3">{{ t('admin.existing_sports') }}</h3>
                                <div class="list-group-container">
                                    <ul class="list-group list-group-flush">
                                        <li v-if="loading.sports" class="list-group-item text-center text-muted">{{ t('common.loading') }}</li>
                                        <li v-for="sport in sports" :key="sport._id" class="list-group-item">
                                            {{ sport.name }}
                                        </li>
                                    </ul>
                                </div>
                ...
                <div class="col-12">
                    <div class="card border-0 shadow-sm">
                        <div class="card-header bg-white py-3">
                            <h2 class="h5 mb-0 text-primary">{{ t('admin.existing_places') }}</h2>
                        </div>
                                ...
                                <table class="table table-hover mb-0 align-middle">
                                    ...
                                    <tbody>
                                        <tr v-if="loading.places">
                                            <td colspan="5" class="text-center py-4">{{ t('common.loading') }}</td>
                                        </tr>
                                        <tr v-for="place in places" :key="place._id">
                                            <td class="fw-medium">{{ place.name }}</td>
                                            <td><span class="badge bg-secondary-subtle text-secondary-emphasis">{{ place.sport?.name || t('admin.js.unknown') }}</span></td>
                                            <td class="text-muted small">{{ place.position.lat }}</td>
                                            <td class="text-muted small">{{ place.position.lng }}</td>
                                            <td>
                                                <button @click="openEditModal(place)" class="btn btn-outline-primary btn-sm me-2">
                                                    {{ t('common.edit') }}
                                                </button>
                                                <button @click="deletePlace(place._id)" class="btn btn-outline-danger btn-sm">
                                                    {{ t('common.delete') }}
                                                </button>
                                            </td>
                                        </tr>
                ...
                <div class="col-lg-6">
                    <div class="card h-100 border-0 shadow-sm">
                        <div class="card-header bg-white py-3">
                            <h2 class="h5 mb-0 text-primary">{{ t('admin.user_reports') }}</h2>
                        </div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-hover mb-0 align-middle">
                                    ...
                                    <tbody>
                                        <tr v-if="loading.reports">
                                            <td colspan="4" class="text-center py-4">{{ t('common.loading') }}</td>
                                        </tr>
                                        <tr v-for="report in reports" :key="report._id">
                                            ...
                                            <td>
                                                <div class="fw-bold">{{ report.reported?.username || t('admin.js.unknown') }}</div>
                                                <small class="text-muted d-block text-truncate" style="max-width: 150px;">{{ report.motivation }}</small>
                                            </td>
                                            <td>
                ...
                <div class="col-lg-6">
                    <div class="card h-100 border-0 shadow-sm">
                        <div class="card-header bg-white py-3">
                            <h2 class="h5 mb-0 text-primary">{{ t('admin.banned_users') }}</h2>
                        </div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-hover mb-0 align-middle">
                                    ...
                                    <tbody>
                                        ...
                                        <tr v-for="user in bannedUsers" :key="user._id">
                                            <td>
                                                ...
                                                <small class="text-muted">{{ user.banReason || 'No reason' }}</small>
                                            </td>
                                            <td class="small">
                                                <div>{{ user.banExpiresAt ? new Date(user.banExpiresAt).toLocaleDateString() : t('admin.js.forever') }}</div>
                                                <div class="text-danger">{{ user.banExpiresAt ? calculateRemainingTime(user.banExpiresAt) : '' }}</div>
                                            </td>
                                            <td>
                                                <button @click="unbanUser(user._id, user.username)" class="btn btn-outline-success btn-sm">Unban</button>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                ...
                <div class="modal-body">
                    <form @submit.prevent="updatePlace">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="edit-sport" v-model="editPlaceForm.sport" required>
                                <option v-for="sport in sports" :key="sport._id" :value="sport._id">
                                    {{ sport.name }}
                                </option>
                            </select>
                        ...
                        <button type="submit" class="btn btn-primary w-100 text-white">{{ t('admin.save_changes') }}</button>
                    </form>
                </div>
            </div>
        </div>
                ...
                <div class="modal-body">
                    <form @submit.prevent="banUser">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="ban-duration" v-model="banUserForm.duration" required>
