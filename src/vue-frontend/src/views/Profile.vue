...
async function handleSendRequest() {
    ...
    try {
        await ProfileAPI.sendFriendRequest(profileUser.value._id, currentUser.value._id);
...
async function handleAcceptRequest() {
    if (!profileUser.value || !currentUser.value) return;
    try {
        await ProfileAPI.acceptFriendRequest(profileUser.value._id, currentUser.value._id);
        window.location.reload();
    } catch (e) { alert("Errore generico"); }
}
...
async function handleRemoveFriend() {
    ...
    if (confirm("Sei sicuro di voler rimuovere questo amico?")) {
        try {
            await ProfileAPI.removeFriend(profileUser.value._id, currentUser.value._id);
            window.location.reload();
        } catch (e) { alert("Errore generico"); }
    }
}
...
async function handleFileChange(event: Event) {
    ...
    if (input.files && input.files[0] && profileUser.value) {
        try {
            await ProfileAPI.uploadProfilePicture(profileUser.value._id, input.files[0]);
            window.location.reload();
...
async function submitReport() {
    ...
    try {
        await ProfileAPI.sendReport(currentUser.value._id, profileUser.value._id, reportMotivation.value);
...
const friendStatus = computed(() => {
    ...
    const isFriend = friends.some((f: any) => (f._id || f) === currentUser.value?._id);
    ...
    const sent = theirReqs.some((r: any) => (r._id || r) === currentUser.value?._id);
    const received = myReqs.some((r: any) => (r._id || r) === profileUser.value?._id);
...
function updateChart() {
    ...
    if (!canvas || selectedSportIndex.value === null || !profileUser.value) return;
    ...
    if (stat.history && stat.history.length > 0) {
        const sortedHistory = [...stat.history].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        ...
        <div class="d-flex flex-column flex-lg-row align-items-start justify-content-center p-4 gap-5">
            ...
            <div class="flex-shrink-0 text-center">
                <div style="position: relative; display: inline-block;">
                    ...
                    <div v-if="isOwnProfile" id="editControls">
                        ...
                        <button v-if="profileUser.profilePicture" @click="handleRemovePic" class="btn btn-danger rounded-circle shadow"
                            style="position: absolute; bottom: 10px; left: 10px; width: 40px; height: 40px; padding: 4px 0; font-size: 1.2rem;">x</button>
            ...
            <div class="flex-grow-1">
                <div v-if="profileUser.sportsElo && profileUser.sportsElo.length > 0" id="eloStatsContainer" class="mb-5">
                    ...
                    <select v-model="selectedSportIndex" @change="updateChart" class="form-select mb-3">
                        <option v-for="(stat, idx) in profileUser.sportsElo" :key="idx" :value="idx">
    ...
    <canvas id="eloChart"></canvas>
            ...
            <div class="flex-shrink-0" style="width: 300px;">
                ...
                <div id="friendsList" class="d-flex flex-wrap gap-2 justify-content-start mb-5">
                    ...
                    <img v-for="friend in friends" :key="friend._id || friend"
                ...
                <h4 class="fw-bold mb-3 border-bottom pb-2 mt-5">{{ t('profile.calendar') }}</h4>
                <div class="card shadow-sm border-0">
                    <div class="card-body p-3">
                        ...
                        <div id="dotsContainer"
                            ...
                            <div v-for="(dot, i) in calendarDots" :key="i"
                                :style="{ width: dot.size + 'px', height: dot.size + 'px', backgroundColor: dot.color, borderRadius: '50%', transition: 'all 0.2s ease' }"
        ...
        <div class="mt-5">
            ...
            <div class="row g-3">
                <div v-if="upcomingGames.length === 0" class="col-12"><span class="text-muted">{{ t('profile.no_upcoming_games') }}</span></div>
                <div v-for="game in upcomingGames" :key="game._id" class="col-md-6 col-lg-4">
                     <div class="card h-100 shadow-sm" style="cursor: pointer;" @click="openGameDetails(game)">
                        <div class="card-body">
                            <div class="d-flex justify-content-between mb-2">
                                <h5 class="card-title fw-bold text-primary">{{ game.sport?.name || 'Sport' }}</h5>
                                <span class="badge" 
                                      :class="{
                                          'bg-danger': game.participants.length >= (game.maxParticipants || 10),
                                          'bg-warning text-dark': game.participants.length / (game.maxParticipants || 10) >= 0.75 && game.participants.length < (game.maxParticipants || 10),
                                          'bg-success': game.participants.length / (game.maxParticipants || 10) < 0.75
                                      }">
                                    <i class="bi bi-people-fill me-1"></i>{{ game.participants.length }}/{{ game.maxParticipants || 10 }}
                                </span>
                            </div>
                            <h6 class="text-muted">{{ game.place?.name || 'Place' }}</h6>
                            <p class="card-text mb-0"><small>📅 {{ new Date(game.date).toLocaleDateString('it-IT') }} at {{ new Date(game.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }}</small></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-5 mb-5">
            ...
            <div class="row g-3">
                <div v-if="pastGames.length === 0" class="col-12"><span class="text-muted">{{ t('profile.no_past_games') }}</span></div>
                <div v-for="game in pastGames" :key="game._id" class="col-md-6 col-lg-4">
                    <div class="card h-100 shadow-sm opacity-75" 
                         :class="{
                             'border-success border-2': game.participants.some((p:any) => (p.user._id || p.user) === profileUser?._id && p.winner),
                             'border-danger border-2': !game.participants.some((p:any) => (p.user._id || p.user) === profileUser?._id && p.winner)
                         }"
                         style="cursor: pointer;" @click="openGameDetails(game)">
                        <div class="card-body">
                            <div class="d-flex justify-content-between mb-2">
                                <h5 class="card-title fw-bold text-primary">{{ game.sport?.name || 'Sport' }}</h5>
                                <span v-if="game.participants.some((p:any) => (p.user._id || p.user) === profileUser?._id && p.winner)" class="badge bg-success">{{ t('profile.won') }}</span>
                                <span v-else class="badge bg-secondary">{{ t('profile.finished') }}</span>
                            </div>
                            <h6 class="text-muted">{{ game.place?.name || 'Place' }}</h6>
                            <p class="card-text mb-0"><small>📅 {{ new Date(game.date).toLocaleDateString('it-IT') }} at {{ new Date(game.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }}</small></p>
                        </div>
