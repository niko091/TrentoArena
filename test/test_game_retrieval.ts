...
describe("Game Retrieval API Tests", function () {
  ...
  before(async () => {
    ...
    await User.deleteMany({
      email: { $in: ["retrieval1@example.com", "retrieval2@example.com"] },
    });
    ...
    await Sport.deleteMany({
      name: { $in: ["RetrievalSportA", "RetrievalSportB"] },
    });
    await Place.deleteMany({
      name: { $in: ["RetrievalPlaceA", "RetrievalPlaceB"] },
    });
    ...
    place1 = await Place.create({
      name: "RetrievalPlaceA",
      sport: sport1._id,
      position: { lat: 0, lng: 0 },
    });
    place2 = await Place.create({
      name: "RetrievalPlaceB",
      sport: sport2._id,
      position: { lat: 0, lng: 0 },
    });
  ...
  it("Step 2: Should filter by SPORT", async () => {
    const res = await request(app)
      .get(`/api/games?sportId=${sport1._id}`)
      .expect(200);
    const relevantGames = res.body.filter(
      (g: any) => g.note && g.note.includes("RETRIEVAL_TEST"),
    );

    // Games 1 and 2 are Sport1
    if (relevantGames.length !== 2)
      throw new Error(
        `Expected 2 games for SportA, got ${relevantGames.length}`,
      );
    if (relevantGames.some((g: any) => g.sport._id !== sport1._id.toString()))
      throw new Error("Got game with wrong sport");
  });

  it("Step 3: Should filter by CREATOR", async () => {
    const res = await request(app)
      .get(`/api/games?creatorId=${user1._id}`)
      .expect(200);
    const relevantGames = res.body.filter(
      (g: any) => g.note && g.note.includes("RETRIEVAL_TEST"),
    );
    ...
    if (relevantGames.length !== 2)
      throw new Error(
        `Expected 2 games for User1, got ${relevantGames.length}`,
      );
  });
  ...
  it("Step 4: Should filter by DATE RANGE", async () => {
    ...
    if (relevantGames.length !== 2)
      throw new Error(`Expected 2 future games, got ${relevantGames.length}`);

    // Verify Past game NOT included
    if (relevantGames.find((g: any) => g.note === "RETRIEVAL_TEST_1"))
      throw new Error("Found past game in future filter");
  });
  ...
  it("Step 5: Should combine filters (Sport + Date)", async () => {
    ...
    if (relevantGames.length !== 1)
      throw new Error(`Expected 1 specific game, got ${relevantGames.length}`);
    if (relevantGames[0].note !== "RETRIEVAL_TEST_2")
      throw new Error("Wrong game identified");
  });
  ...
  it("Step 6: Should not leak User Passwords", async () => {
    ...
    if (game.creator.password)
      throw new Error("Password leaked in creator population");
  });
});
