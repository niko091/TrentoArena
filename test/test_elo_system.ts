...
describe("ELO System Tests", function () {
  ...
  it("Step 1: Should update ELOs correctly after a game", async () => {
    // Create 2 users
    const winner = {
      username: `${ELO_TEST_PREFIX}_Winner`,
      email: "winner@elo.com",
      password: "password",
    };
    const loser = {
      username: `${ELO_TEST_PREFIX}_Loser`,
      email: "loser@elo.com",
      password: "password",
    };
    ...
    const wEloEntry = wUser.sportsElo?.find(
      (e) => e.sport.toString() === sportId,
    );
    const lEloEntry = lUser.sportsElo?.find(
      (e) => e.sport.toString() === sportId,
    );
    ...
    // Verify History
    if (!wEloEntry.history || wEloEntry.history.length === 0)
      throw new Error("Winner History not created");
    if (!lEloEntry.history || lEloEntry.history.length === 0)
      throw new Error("Loser History not created");

    const wHist = wEloEntry.history[0];
    if (wHist.elo !== wEloEntry.elo)
      throw new Error("Winner History ELO mismatch");
