...
describe('Report API Tests', () => {
    ...
    before(async () => {
        ...
        await User.deleteMany({ email: { $in: ['api_reporter@example.com', 'api_reported@example.com'] } });
    ...
    it('Step 1: Should create a new report', async () => {
        ...
        if (res.body.report.motivation !== 'Spamming in chat') throw new Error('Incorrect motivation');
    });
