...
describe('Stats API Tests', function () {
    ...
    before(async () => {
        ...
        if (mongoose.connection.readyState === 0) {
            const testUri = process.env.MONGO_URI_ || 'mongodb://localhost:27017/trentoArena';
        ...
        await Sport.deleteMany({ name: { $in: ['Stats_Sport_1', 'Stats_Sport_2'] } });
        await Place.deleteMany({ name: { $in: ['Stats_Place_1', 'Stats_Place_2'] } });
        ...
        // Create Places
        const p1 = await Place.create({ name: 'Stats_Place_1', sport: s1._id, position: { lat: 0, lng: 0 } });
        const p2 = await Place.create({ name: 'Stats_Place_2', sport: s2._id, position: { lat: 0, lng: 0 } });
        placeId1 = p1._id.toString();
        placeId2 = p2._id.toString();

        // Create Games (Finished)
        const g1 = new Game({
            sport: sportId1, place: placeId1, creator: creatorId,
            date: new Date('2023-01-01T10:00:00Z'), note: 'API_STATS_TEST', isFinished: true, maxParticipants: 2
        });
        const g2 = new Game({
            sport: sportId1, place: placeId1, creator: creatorId,
            date: new Date('2023-01-01T12:00:00Z'), note: 'API_STATS_TEST', isFinished: true, maxParticipants: 2
        });
        const g3 = new Game({
            sport: sportId1, place: placeId2, creator: creatorId,
            date: new Date('2023-01-02T10:00:00Z'), note: 'API_STATS_TEST', isFinished: true, maxParticipants: 2
        });
        const g4 = new Game({
            sport: sportId2, place: placeId2, creator: creatorId,
            date: new Date('2023-02-01T10:00:00Z'), note: 'API_STATS_TEST', isFinished: true, maxParticipants: 2
        });
        // Unfinished game (should be ignored)
        const g5 = new Game({
            sport: sportId1, place: placeId1, creator: creatorId,
            date: new Date(), note: 'API_STATS_TEST', isFinished: false, maxParticipants: 2
        });
    ...
    it('GET /api/stats/chart-data - Should return counts by date for SPORT', async () => {
        ...
        if (res.body['01/01/2023'] !== 2) throw new Error(`Expected 2 games on 01/01/2023, got ${res.body['01/01/2023']}`);
        if (res.body['02/01/2023'] !== 1) throw new Error(`Expected 1 game on 02/01/2023, got ${res.body['02/01/2023']}`);
        if (Object.keys(res.body).length !== 2) throw new Error('Expected 2 date entries');
    });
    ...
    it('GET /api/stats/chart-data - Should return counts by date for PLACE', async () => {
        ...
        if (res.body['02/01/2023'] !== 1) throw new Error(`Expected 1 game on 02/01/2023, got ${res.body['02/01/2023']}`);
        if (res.body['01/02/2023'] !== 1) throw new Error(`Expected 1 game on 01/02/2023, got ${res.body['01/02/2023']}`);
    });
    ...
    it('GET /api/stats/top-entities - Should return top SPORTS', async () => {
        ...
        if (!s1 || s1.count !== 3) throw new Error('Stats_Sport_1 count should be 3');
        if (!s2 || s2.count !== 1) throw new Error('Stats_Sport_2 count should be 1');
    });
    ...
    it('GET /api/stats/top-entities - Should return top PLACES', async () => {
        ...
        if (!p1 || p1.count !== 2) throw new Error(`Stats_Place_1 count should be 2, got ${p1 ? p1.count : 'undefined'}`);
        if (!p2 || p2.count !== 2) throw new Error(`Stats_Place_2 count should be 2, got ${p2 ? p2.count : 'undefined'}`);
    });
});
