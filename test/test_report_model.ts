...
describe('Report Model Tests', () => {
    ...
    before(async () => {
        ...
        await User.deleteMany({ email: { $in: ['reporter@example.com', 'reported@example.com'] } });
    });
    ...
    it('Step 1: Should save and retrieve a report', async () => {
        ...
        if (foundReport.motivation !== 'Inappropriate behavior') throw new Error('Motivation mismatch');
        if (foundReport.reporter.toString() !== reporter._id.toString()) throw new Error('Reporter ID mismatch');
    });
