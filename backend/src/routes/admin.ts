...
router.post('/ban', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (duration !== 'forever') {
            ...
            if (duration === '1d') now.setDate(now.getDate() + 1);
                ...
                if (!isNaN(hours)) {
                    ...
                    return res.status(400).json({ message: 'Invalid duration format. Use 1d, 1w, 1m, or number of hours.' });
                }
            }
            banExpiresAt = now;
        }
        ...
        res.json({ message: `User ${user.username} has been banned`, banExpiresAt });
    } catch (err) {
        console.error(err);
...
router.get('/banned-users', async (req: Request, res: Response) => {
    try {
        const bannedUsers = await User.find({ isBanned: true })
            .select('username banExpiresAt banReason');
