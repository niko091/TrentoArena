\chapter{Requisiti Funzionali}
    \section{Requisiti funzionali comuni a tutti gli utenti}
        \begin{rfList}
            ...
            L'utente compila un modulo che richiede: nome utente (che deve essere unico nel sistema), data di nascita, e-mail, password e conferma password.
            ...
            Il sistema recupera l'e-mail dall'account Google.
		...
		Il feed deve mostrare un elenco di partite disponibili e le attività degli amici, ordinate cronologicamente (dalla data più vicina).
		L'utente deve poter filtrare questo feed in base a sport, luogo, attività ("Sfide create", "Sfide completate", "Nuovo amico aggiunto") e contesto ("Partite create da me"). La pagina deve contenere un widget per visualizzare il proprio profilo e le statistiche principali (numero di amici, numero di partite giocate, posizione in classifica)

            \rfItem \textbf{Ricerca globale:} 
            Il sistema deve fornire una funzionalità di ricerca (tramite una barra di ricerca nell'header).
            L'utente inserisce una stringa di testo, il sistema deve cercare la stringa nei seguenti ambiti: utenti (per nome utente), luoghi (per nome struttura) e partite (per sport, luogo e data).
            ...
            Un utente deve poter creare una nuova partita, per farlo compila un form che richiede: sport (selezionato da un elenco predefinito), luogo (selezionato tramite la mappa), data e orario, tipo di partita (individuale / a squadre), note aggiuntive (campo di testo libero).

            \rfItem \textbf{Visualizzazione mappa delle strutture:} 
            Il sistema deve fornire una mappa interattiva con degli indicatori in corrispondenza delle strutture sportive pubbliche, cliccando su una struttura si apre una scheda informativa sulla struttura che mostra le partite in programma. L'utente deve poter filtrare le strutture visualizzate in base allo sport.
            ...
            Ogni utente deve avere una pagina profilo pubblica che mostra: l'elenco degli amici, un grafico del punteggio ELO nel tempo, le partite passate e future visualizzate sia sottoforma di lista che in un calendario.
            \newline
            ...
            \rfItem \textbf{Invio richista di amicizia:}
		  Il sistema deve fornire un meccanismo per inviare richieste di amicizia ad altri utenti. Sul profilo di ogni utente deve essere presente un pulsante "Invia richiesta di amicizia", cliccandolo la richiesta verrà mandata all'utente (RF10)
    ...
    \section{Requisiti funzionali per i creatori di sfide}
        \begin{rfList}
		  ...
		  Il sistema deve permettere all'utente che ha creato una partita di inserirne il risultato (Solo dopo l'orario di inizio della partita).
        \end{rfList}
